...
const Experience = () => {
  ...
  return (
    ...
    >
        ...
        <ul className="space-y-12">
          {experiences.length === 0 ? (
            ...
            experiences.map((exp) => (
              <li
                key={exp.id}
                className={`group relative rounded-lg p-6 shadow-lg border transition-all duration-300 
                ${darkMode ? "bg-black border-gray-700" : "bg-white border-gray-200"} 
                w-full md:w-3/4 mx-auto md:ml-20`}
              >
                {/* Timeline Dot (no constant glow, glows on hover) */}
                <div
                  className={`absolute -left-6 md:-left-12 top-8 w-5 h-5 rounded-full border-4 transition-all duration-500
                  ${darkMode ? "bg-green-700 border-gray-900" : "bg-green-500 border-gray-50"}
                  group-hover:shadow-[0_0_15px_4px_rgba(34,197,94,0.7)]`}
                ></div>
                ...
                {/* Company */}
                <p
                  className={`flex items-center font-medium mb-7 transition-colors duration-500 ${
                ...
                {/* Description List */}
                <ul
                  className={`text-sm leading-relaxed space-y-2 transition-colors duration-500 ${
      ...
      {/* Footer */}
      <div
        className={`mt-10 text-center transition-colors duration-500 ${
          darkMode ? "text-gray-400" : "text-gray-600"
        }`}
      >
